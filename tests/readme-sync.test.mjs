...
for (const [i, f] of [README_EN, README_ZH].entries()) {
  ...
  // 它回「已经是最新版本」然后什么都不做。
  //
  // ⚠️ **M3d 追记：这条判据的理由换掉了一半，而判据本身一个字没改。** 写它的时候本仓库的
  // `version` 从第一个 commit 起就是 `0.1.0`，所以那条命令**跑了也白跑**；M3d 之后版本号
  // 跟着每一次推 `main` 动（纪律在两份 README 的「开发 / Development」那一节），
  // **于是它从「会骗人的命令」变成了「正常该跑的命令」**。两种情形下它都必须出现在安装
  // 说明里——理由换了，结论没换，所以判据不动。
  //
  // **一个照着安装说明装完的人，下一步最可能敲的就是这条命令。**
  ...
  test(`${f} 的安装那一节写着 claude plugin update 这条命令`, () => {
    ...
    assert.ok(
      ...
      `${f} 的安装那一节里没有 claude plugin update。docs/19 §8.4.3 实测到的是：` +
        '这条命令比的是 plugin.json 里的 version 而不是 commit——所以它拿不拿得到新代码，' +
        '完全取决于本仓库有没有挪版本号（M3d 起这是一条纪律，见两份 README 的「开发」那一节）。' +
        '装完的人下一步最可能敲的就是它。\n' +
  ...
  // M3d。**推 `main` 是这个仓库唯一的发布事件**，而 `claude plugin update` 比的是
  // `plugin.json` 里的 `version`——**一次不挪版本号的推送，对已经装好的人完全无声，
  // 而 CLI 还会主动告诉他们「已经是最新版本」**。纪律正文落在「开发 / Development」
  // 那一节（为什么是那一节，`docs/19` §9.3）。
  //
  // 下面借的是 `installSectionOf`——它并不是安装那一节专用的，是个「从这一对标题起、
  // 到下一个 `## ` 止」的通用抽取器，而它**停在下一个小节**这一条已经有自检锚钉着。
  //
  // ⚠️ **这条钉的是「那一节里提到了那个文件和那个字段」，不是「纪律被遵守了」。**
  // 上限与第七节那两条（`--scope local`、`claude plugin update`）一样，理由也同一条：
  // 这一族没有更强的、不在仓库里造第二份拷贝的判据。
  ...
  test(`${f} 的开发那一节写着发布纪律要挪哪个文件里的哪个字段`, () => {
    ...
    for (const needle of ['.claude-plugin/plugin.json', 'version']) {
      assert.ok(
        section.includes(needle),
        `${f} 的 ${devHeading} 那一节里没有 ${needle}。这一节从 M3d 起承载着本仓库唯一的` +
          '发布纪律：推 main 就是发布，所以每次推 main 都要挪 .claude-plugin/plugin.json 的 version。\n' +
          '  docs/19 §8.4.3 实测过，那个字符串是 claude plugin update 唯一会去比的东西——' +
          '不挪它，已经装好的人一个字节都收不到，而且是无声的。\n' +
          '  **把这条纪律从 README 里删掉，不会有任何别的判据变红**（docs/16 §3.7 那一族），' +
          '代价是下一轮推上去的修复谁也收不到。\n' +
          '  如果真的是要改——比如改成用 tag 或 CHANGELOG 来标记发布——那要先去读 docs/19 §9.2，' +
          '那里写着为什么今天没有机械判据、以及什么会让那个答案改变，再回来改这一条。',
      )
    }
  })
...
// 参数值留着，那一刀才红。
//
// ⚠️ **这里原来还有第三道网（「丢掉任何非 ASCII 的 token」），已删，两条理由**
// （M2d 定向复评 N3 / N4）：
//   · **它是死代码。** 逐网关掉实测：关第一道红 2、关第二道红 1、
//     **关第三道 629 / 0 一条不红**。中文那份的占位符 `<插件>@<市场>` 以 `<` 开头，
//     **第一道网就吃掉了**；而下面 T2 的第二个断言曾被注释当成第三道网的锚
//     ——**那个锚钉错了层**（`docs/16` §3.2）。
//   · **就算它哪天真的命中，行为也是错的。** 它只丢**一侧**：中文那个 token 被丢、
//     英文对位的那个 ASCII token 留着，两份当场对不上——那是一次**假红**，
//     而假红比不管更贵。
//   · 它还有一个纯工具面的副作用：那条字符类是用**字面 NUL 与 DEL 字节**写进源码的
//     （`\u0000` 这类转义被写文件的工具当场解释掉了），于是 `grep` 把本文件判成
//     **二进制**、不加 `-a` 只打 `Binary file … matches`——**而这个仓库的评审方法
//     本身高度依赖 grep 扫散文与注释**。同族的坑记在 `docs/16` §3.8。
...
//
// ⚠️ 下面三个断言各自钉的是**哪一道网**，写清楚（M2d 定向复评 N3：第一版把第二个
// 断言当成了「非 ASCII 那道网」的锚，而它其实走的是第一道网，锚钉错了层）：
// 第一、二个都走**第一道网**（`startsWith('<')`）——**两种语言的占位符都以 `<` 开头**，
// 中文那个不需要额外一道网；第三个走**第二道网**（路径）。
// 归一化今天只有这两道，两道都有锚，**没有第三道可钉**。
...
// **两份一起写同一个错，这一条一声不响。** 这不是漏，是它的定义：
// 镜像判据的真源是两份自己。**「两份一致」与「两份对」是两件事。**
//
// ⚠️ **第一版这里举的例子是假的，更正留着不抹**（M2d 定向复评 N2）。
// 原话是「两份都改成 `--scope user`——这一条一声不响」。**实测 627 / 2**：
// 红的是第七节那两条「安装那一节写着 `--scope local`」。
// 同一句措辞在上面 F6 那段里是**对的**，因为那里明写着
// **「段内别处仍留着 `--scope local`」**；**这里漏掉了那个限定，字面就成了假话。**
// 形状与本轮 F1 同族：**结论对、范例假**，而范例比结论传得远。
//
// **今天还站得住的例子**（下面那条黑名单上线之后重测过）：两份**同时**把
// `agent-team@agent-team-marketplace` 写成一个并不存在的市场名——仍然镜像、
// 不是禁用作用域，**全绿**。那正是「两份一致 ≠ 两份对」。
// 而当年用来说明这一点的那一刀（两份只把 install 那条改成 `--scope project`）
// **今天已经红了**——下面那条禁用作用域黑名单把它接住了。**盖不住的那一半被啃掉了一块，
// 不是全部。**
//
...
// ---------------------------------------------------------------------------
// 十、禁用作用域黑名单 —— 在「正确」那个方向上拿得到的那一点
// ---------------------------------------------------------------------------
//
// M2d 定向复评 N5。第九节钉的是**镜像**，它钉不住「两份一起写同一个错」
// （那一节末尾写清楚了）。而**有一条判据不需要回答「哪几条算推荐」**，
// 所以它躲开了那条更强判据 X 的全部代价：
...
//
// 它**不替代**第九节：镜像管「两份别分叉」，黑名单管「别一起掉进这两个坑」。
// 两条各盖一块，**合起来仍然盖不住「两份一起写了别的错」**（第九节末尾那个例子）。
//
// ⚠️ **今天这两个串在两份 README 的命令行里各 0 次命中**，所以锚**不能**用
// 「派生集合非空」那一款（`docs/16` §3.3：那一款一上线就红，而这里 0 命中恰恰是对的）。
// 用**已知违规样本**那一款：拿一个合成的违规输入，要求判据认得出来。
const FORBIDDEN_SCOPES = ['--scope user', '--scope project']

// 主判据与锚共用这一份。复用第九节的 `commandLinesOf`：**只看命令行，不看散文**
// ——散文里讨论这两个作用域（比如「不要用 `claude plugin enable`」那一段的邻居）
// 是正当的，**教人敲**才是问题。
function forbiddenScopesIn(text) {
  return commandLinesOf(text).flatMap((cmd) =>
    FORBIDDEN_SCOPES.filter((s) => cmd.includes(s)).map((s) => `${s}  <-  ${cmd}`),
  )
}

// ⭐ 已知违规样本锚：0 命中的判据，锚只能钉在样本上。
test('锚：禁用作用域判据认得出已知违规样本——真文件今天 0 命中，锚钉不到真文件上', () => {
  assert.deepEqual(forbiddenScopesIn('装的时候用 `claude plugin install x --scope user` 就行'), [
    '--scope user  <-  claude plugin install x --scope user',
  ])
})

// ⭐ 反向锚：散文里提到这两个词不算违规，只有**命令行里**才算。
// 少了这条，判据会退化成「这两个字符串不许出现在 README 里」——
// 那会把「说清楚为什么不要用它」这件正当的事一起禁掉。
test('锚：散文里提到禁用作用域不算违规——判据只看命令行', () => {
  assert.deepEqual(forbiddenScopesIn('不要用 --scope user，本项目禁了它。'), [])
})

test('前置条件：FORBIDDEN_SCOPES 非空——空清单会让下面两条在零个禁令上恒绿', () => {
  assert.ok(
    FORBIDDEN_SCOPES.length > 0,
    'FORBIDDEN_SCOPES 被清空了。下面两条会对着零个禁令逐条迭代，也就是什么都不拦——' +
      '而它们仍然是绿的（docs/11 §5.20 第四节记的就是这一族）',
  )
})

for (const f of [README_EN, README_ZH]) {
  test(`${f} 的命令行里没有本项目明令禁止的那两个作用域`, () => {
    assert.deepEqual(
      forbiddenScopesIn(read(f)),
      [],
      `${f} 里有命令行教人用本项目自己禁掉的作用域。\n` +
