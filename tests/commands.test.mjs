...
// 角色名去对花名册查——排除掉这个集合。
//
// 裁定「命令名排除集单一真源」（M2b Task 4 补轮）：这个排除集原来是本文件里的一行
...
//
// ⭐ 裁定「先解决的那一侧留缺口」（M2b Task 4 最后一轮）：这一条**此前没有任何正向锚**。
...
// 东西看得见（docs/11 §5.15）。
//
// 发现它的方式也记一笔：不是读出来的，是**角色侧的变异顺带照出来的**——裁定「命令名排除集单一真源」的
...
// ⭐ 正向锚（裁定「先解决的那一侧留缺口」）。⚠️ 钉的是**排除之后**那一半——不是 `text.match(...).length > 0`
...
// ⭐ 裁定「锚跟着收窄往里挪」（M2b Task 4 收尾轮）：这条判据**收窄三次**，而此前**没有任何正向锚**。
//
// 它已经因为同一类原因接近空转**两次**：
...
// 命令名不是角色名，这件事的单一真源是 tests/helpers/command-names.mjs（裁定「命令名排除集单一真源」与「先解决的那一侧留缺口」）。
...
// 着。**这正是裁定「先解决的那一侧留缺口」命名的形状**：先解决问题的那一侧，最容易在问题的第二半上
...
// ⭐ 裁定「paths 禁令要有守卫」（M2b Task 4 补轮，2026-09-20）：`/at-init` 正文里「不要给某某角色建 paths
...
test('commands/at-init.md 必须逐个点名禁止给「不认领路径的角色」建 paths 键（裁定「paths 禁令要有守卫」）', () => {
