...
// 非空（那个恒真）。
//
// ⚠️ **M3j：这条锚从「非空」升成了「身份」，而升它的理由是实测出来的。**
// 「非空」挡得住的只有集合**塌到零**，挡不住**少一个**——`ROLES_WITH_EDGES` 是从
// `roster.json` 派生的，哪个角色的 `can_delegate_to` 被清空，它就**直接退出遍历**：
// 上面那条「有边就必须有 Agent(...)」与下面那条 H1 第一道，对它一声不吭地不再检查，
// 而那份正文里的话一个字没变，也没有任何东西说它现在没人管。
// 实测（`docs/11` §5.32 变异 B/C，**下面这两句说的是本条升级之前的仓库**）：
// 把 `at-product` 的 `can_delegate_to` 清空，当时全仓只红 `tests/reach.test.mjs`
// 的拓扑快照一条，而那一条的失败文案说的是「回来确认触达有没有被放大」——
// **照它说的做完、把期望拓扑一起更新，当时就是 799 / 0 全绿**，
// 此时 `at-product.md` 的 `tools:` 里还挂着 `Agent(...)`、正文里还写着 H1 那句话，
// 而花名册里它一条边都没有。**今天这两刀都红在本条上**（变异 B / C 各自的结果
// 记在那一节的清单里）。
//
// ⚠️ **上面那段注释原来还有半句，逐字是「也不是 EXPECTED_DELEGATORS 非空（那是另一份
// 数据，改不到一起）」——它拒的是拿那份清单做「非空」锚，那一半今天仍然对。**
// 这里换上的是**身份**锚：正因为它是另一份数据，两边「改不到一起」才应该当场红，
// 而不是各自静默。`EXPECTED_DELEGATORS` 自己的失败文案逐字写着「新增或删掉一个会派发的
// 角色时必须回来同步这份清单」——那正是这条锚要的那个动作。
//
// ⚠️ **它与上面那条主判据的关系要说清楚，别当成两份拷贝**：这条锚 + 上面那条
// 「agents/ 目录下恰好是这三份带 Agent(...) 白名单的正文」合起来，确实蕴含主判据的
// ⊆ 方向。主判据留着，因为**红的时候它逐个角色点名，并且说出后果**（那个角色在会话里
// 根本拿不到派发工具）；锚红的时候说的是另一件事——**遍历范围本身变了**。
// 两条失败时给的是两种诊断，不是同一句话说两遍。
test('锚：roster.json 里有边的角色，恰好是 EXPECTED_DELEGATORS 那几份——少一个都红，不只是塌到零才红', () => {
  assert.deepEqual(
    ROLES_WITH_EDGES.map((r) => `${r}.md`),
    [...EXPECTED_DELEGATORS].sort(),
    `roster.json 里「有 can_delegate_to 边、且不是 ${MAIN}」的角色算出来是 ` +
      `${JSON.stringify(ROLES_WITH_EDGES)}，与 EXPECTED_DELEGATORS ` +
      `${JSON.stringify([...EXPECTED_DELEGATORS].sort())} 对不上。\n` +
      '  **这条红了，不是让你照新值把清单改一改了事。**上面那条「有边就必须有 Agent(...)」\n' +
      '  与下面那条「有边的正文都写着 H1 第一道」**只遍历这个集合**：\n' +
      '  少一个名字 = 那份正文从此退出了这两条的检查范围，而它正文里的话一个字没变。\n' +
      '  · 少一个：先确认那个角色是真的不再派发任何人（那时 agents/ 下那份正文的\n' +
      '    「你能派谁」一节与 tools: 里的 Agent(...) 要一起清掉，EXPECTED_DELEGATORS 才跟着改）；\n' +
      '  · 多一个：新长出边的那个角色，正文里 H1 第一道那句话写上了吗？\n' +
      '  ⚠️ 只更新 tests/reach.test.mjs 的拓扑快照**不够**——那条守的是「触达有没有被放大」，\n' +
      '  不是「这两条判据还在不在看这个角色」。背景与实测在 docs/11 §5.32。',
  )
})
