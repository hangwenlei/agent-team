...
（可用班底减去这一趟叫到的人）。两边**会重叠**——一个整趟没被叫到过的角色，被裁掉之后
两边都有它（**不是每一个都会**：`trimmed` 按角色名判，某一段裁掉、另一段又派了的角色
进了 `roster`，于是只出现在这一边）。所以「没被叫过」那一行要把重叠的那些标出来：
**剩下没被标的，才是「没人注意到」的那一种**，而那正是规格 §4.2 ④ 要防的。读不到 `trimmed`（M3a 之前落盘的 run 没有这个字段）就把「主动裁掉」写成
