...
4. 把这一段真正叫到的角色累加进 `roster`。**`never_invoked` 不要在这里逐段累加**
   ...
   **不写 `trimmed`，被你裁掉的那个人就会出现在那条回传里。**
   ...
   ⚠️ **只给那条回传真的点到名的人写 `trimmed`，不要照着「这一段还有谁没来」预先补。**
   你自己也是某几段的产出角色，但你**不在** `available_roles` 里
   （`/agent-team:at-init` 明令不写它），**那条判据从头到尾不会点你的名**。
   把自己写进 `trimmed` 会被状态校验放行——它挑不出毛病，因为你确实是那几段的产出角色
   ——然后无声地吃掉**别人**那几条本该报的：`trimmed` 按角色名判，**一条记录盖住
   那个角色的每一段**。同理，没真裁过的人也别写进去。
