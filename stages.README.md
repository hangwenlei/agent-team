...
**下一段**的缺失产物把**这一段**顶回去，顶到平台放弃为止，然后平台
静默放行——一路纯噪音的拦截，还会把 H5b 唯一的重试预算消耗光。
（**顶多少下不写在这里**：那个计数不是常数，单一真源是 `hooks/lib/retry-budget.mjs`，
判据是 `tests/retry-budget-single-source.test.mjs`。这一句此前写的是中文数字，
于是按词形扫同一份知识时它被漏掉——`docs/16` §3.4 的实物，逐句在 `docs/11` §5.29。）
...
——先改的是这一边，那一边是随后一并补的。**本仓库最稳定的失败模式就是
「更正只作用到它逐字点名的那一句上」**，所以改一处的时候去看另一处。
