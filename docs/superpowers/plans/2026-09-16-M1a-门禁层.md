...
- Produces:
  ...
  - `gate.mjs` 的 `emitDeny(reason, event)`：`hookEventName` 由调用方传入，不再硬编码
...
- [ ] **Step 5: 注册 stop-gate 与 writepath 以便分派可测**
...
`writepath` 的 hook 注册已在 Task 1 完成，本任务只接判定逻辑，不必再动 `hooks/hooks.json`。
...
```js
  if (CHECK === 'stop-gate' || CHECK === 'deliverable') {
    ...
    if (CHECK === 'stop-gate') {
      // H5b：真拦截。平台会重试约 8 次，每次角色都能看到这条理由。
      emitDeny(r.reason, spec.event)
    } else {
