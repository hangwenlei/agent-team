...
PM 那条 `Agent(...)` 白名单是**硬的**（实测：非白名单类型报 `not found`，注册表被过滤）。
产品经理与架构师的白名单是**软的**——官方 sub-agents 文档明文：subagent 定义里
括号内的类型列表被忽略。第二层以下的派发约束只能靠 hook。
...
| # | 项 | 状态 | 影响 |
|---|---|---|---|
| U1 | `agent` 键是否接受插件命名空间 id（`agent-team:at-pm`） | 未测 | 若不接受，需换写法 |
| U2 | 插件提供的 agent 能否被主线程白名单按裸名引用 | 未测 | 影响 PM 的 `tools:` 写法 |
| U3 | hook 在 Windows 下能否稳定拿到 `agent_type` 并 deny | 未测 | **五个 hook 的共同地基** |
| U4 | subagent 能否在一条消息内并发 spawn 多个 subagent | 未测 | S3/S5 并行的前提；若不行则退化为串行 |
