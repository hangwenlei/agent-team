...
`available_roles` 里的角色，在这个项目里**从来没上过队**——不存在一个关于它的「裁剪」
决定，要求为它写一条 `trimmed` 等于要求把一个不存在的决定写下来，而这条机制的立论
恰恰是「把**已经存在**的决定变成可机器读的」。**最要紧的一格是 `at-pm` 自己**：
它是某几段的产出角色（`role` 就是它），却按规定**不写进** `available_roles`
——「它是这一趟的驱动者，不参与『有没有被叫到』的统计」。
**所以它从来不该被这条判据点名，也绝不该被写进 `trimmed`**：形状校验会放它过去
（它确实是那几段的产出角色），而 `trimmed` 按角色名判、**一条记录盖住那个角色的每一段**，
写进去等于无声地吃掉别人那几条本该报的。
